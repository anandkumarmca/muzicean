...
class OverdueBatchEdit extends React.Component {
    scrollRef = null;
    currentMonth = 0;
    selected = null;
    info = {};
    ...
    selectDated = moment(this.props.route.params.batch.attdate).format('D');
    ...
    componentDidMount() {
        ...
        let startdate = moment(this.props.route.params.batch.attdate).format('D');
        let startmonth = moment(this.props.route.params.batch.attdate).format('M');
        let startyear = moment(this.props.route.params.batch.attdate).format('Y');
        ...
        this.setState({
            batchname: this.props.route.params.batch.batchname,
            course: this.props.route.params.batch.course,
            stime: this.props.route.params.batch.stime,
            etime: this.props.route.params.batch.etime,
        });

        POST(SERVER + 'users/checkHoliday.json', {
            selectedate: this.props.route.params.batch.attdate,
            centre_id: this.props.route.params.batch.centre_id,
        }).then(response => {
        ...
        this.getalertshow(
            this.props.route.params.batch.schedule_id,
            this.props.route.params.batch.schedule_date_id,
        );

        POST(SERVER + 'users/overdueScheduleMore.json', {
            id: this.props.route.params.batch.schedule_id,
            dateid: this.props.route.params.batch.schedule_date_id,
        }).then(response => {
            ...
            if (response == null) {
                ...
                Object.keys(this.state.studentListing).map(key => {
                    this.state.studentListing[key] = { ...this.state.studentListing[key] };
                    this.state.student.push(this.state.studentListing[key]);
                    var at = this.state.studentListing[key].schedule_attendee_id;
                    var r = this.state.studentListing[key].faculty_comment;
                    this.state.userRemark[at] = r;
                    if (this.state.studentListing[key].statusSet == 0) {
                        i++;
                    }
                });
    ...
    checkPresent() {
        ...
        if (selected == '') {
            Alert.alert('Mark Present Alert', 'Please select at least one student..!!');
        } else {

            selected.forEach(value => {
                if (
                    this.state.student[value].sstatus != 'freeze' &&
                    this.state.student[value].sstatus != 'hold' && this.state.student[value].statusSet == 0
                ) {
            ...
            this.setState({ student: this.state.student, selectedClass: [], selectedCheck: false });
    ...
    checkAbsent() {
        ...
        if (selected == '') {
            Alert.alert('Mark Absent Alert', 'Please select at least one student..!!');
        } else {
            selected.forEach(value => {
                if (
                    this.state.student[value].sstatus != 'freeze' &&
    ...
    markFA() {
        ...
        this.props.navigation.navigate(
            ...
            {
                id: this.props.route.params.batch.schedule_id,
                data: this.props.route.params.batch,
                dateid: this.props.route.params.batch.schedule_date_id,
                date: this.props.route.params.batch.attdate,
                time: this.props.route.params.batch.stime + ' - ' + this.props.route.params.batch.etime,
                batchname: this.props.route.params.batch.batchname,
                _centrename: this.state.centre_name,
    ...
    reschedule() {
        ...
        if (selected == '') {
            ...
            if (selected.length == 1) {
                selected.forEach(value => {
                    if (this.state.student[value].statusSet == 0) {
    ...
    submitOverdue() {
        ...
        if (this.flag != 1) {
            ...
            POST(SERVER + 'users/overdueDetails.json', {
                sid: this.props.route.params.batch.schedule_id,
                did: this.props.route.params.batch.schedule_date_id,
                coursename: this.props.route.params.batch.course,
                batchname: this.props.route.params.batch.batchname,
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if (this.props.route.params.batch.attdate !== prevProps.route.params.batch.attdate) {
            this.componentDidMount();
        }
    }
            ...
            <KeyboardAvoidingView
                ...
                <View
                    ...
                    <Animatable.View
                        ...
                        <View style={{ flexDirection: "row", paddingTop: 5, paddingBottom: 35, paddingLeft: 10, backgroundColor: PROPERTY.bwColor }}>
                            <TouchableOpacity onPress={() => { this.props.navigation.reset({ index: 0, routes: [{ name: 'OverdueList' }] }); }}>
                            ...
                            <View style={{ flex: 91, alignItems: 'flex-start', justifyContent: "center", paddingTop: 20, paddingRight: 30 }}>
                                <Text style={{ ...STYLES.fontLarge, color: PROPERTY.selectedColor, fontWeight: 'bold' }}>Overdue Batch Edit</Text>
                            </View>
                        </View>
                    </Animatable.View>
                    {/* <Users /> */}
                            ...
                            {!this.state.isLoading && (
                                <View>
                                    ...
                                    <View style={{ flexDirection: 'row', marginTop: 25 }}>
                                        <View style={(this.state.setHoliday == 0) ? { flex: 51 } : { flex: 39 }}>
                                            {!(
                                        ...
                                        <View style={(this.state.setHoliday == 0) ? { flex: 49 } : { flex: 61 }}>
                                            <View style={{ flexDirection: 'row' }} >
                                                ...
                                                {this.state.faShow == true && (
                                                    <TouchableOpacity
                                                        ...
                                                        <View
                                                            ...
                                                            <Text style={{ ...STYLES.scheduleBtnText }}>FA</Text>
                                                        </View>
                                                    </TouchableOpacity>
                                                )}
                                                <TouchableOpacity
                                                ...
                                                <TouchableOpacity
                                                    ...
                                                    <View
                                                        ...
                                                        <Icon name={'exchange'} size={18} color={'#E8E8E9'} />
                                                    </View>
                                                </TouchableOpacity>
                                                {this.state.setHoliday == 1 && (
                                                    <TouchableOpacity onPress={this.markHoliday}>
                                                        <View
                                                            ...
                                                            <Text style={{ ...STYLES.scheduleBtnText }}>MH</Text>
                                                        </View>
                                                    </TouchableOpacity>
                                                )}
                                            </View>
                    ...
                    <Modal
                        ...
                        <View style={{ paddingTop: 0, paddingLeft: 20, marginTop: 10 }}>
                            <Text style={{ fontSize: 18, fontWeight: '500' }}>Do you want to Proceed to Mark FA for all students in this batch</Text>
                        </View>
                        <View style={{ flexDirection: 'row', paddingTop: 10 }}>
                    ...
                    <Modal
                        ...
                        <View style={{ paddingTop: 0, paddingLeft: 20, marginTop: 10 }}>
                            <Text style={{ fontSize: 18, fontWeight: '500' }}>Do you want to Proceed to Mark Holiday for all students in this batch</Text>
                        </View>
                        <View style={{ flexDirection: 'row', paddingTop: 10 }}>
                            <View style={{ flex: 9, padding: 5 }}></View>
                            <View style={{ flex: 40, paddingTop: 5 }}>
                    ...
                    <Modal visible={this.state.confirmModal} dismissable={false}
                        contentContainerStyle={{ ...STYLES.modalDialog, backgroundColor: PROPERTY.background, marginTop: 15 }}>

                        <TouchableOpacity onPress={() => { this.setState({ confirmModal: false, }) }} >
                            <Image source={require('../../Assets/icons/close.png')} style={{ resizeMode: 'contain', width: 40, height: 40, marginLeft: 325, marginTop: 8 }} />
                        </TouchableOpacity>

                        <View style={{ paddingTop: 0, paddingLeft: 20 }}>
                            <Text style={{ fontSize: 20, fontWeight: '500', marginTop: -25, color: PROPERTY.overdueColor }}>Confirmation Alert</Text>
                            <Text style={{ fontSize: 16, fontWeight: '500', marginTop: 15, paddingLeft: 5 }}>Mark attendance request for :</Text>
                        </View>
                        <View style={{ width: width - 80, marginLeft: 15, marginTop: 10 }}>
                            {this.state.student.map((value, index) => {
                                return (
                                    (value.statusSet == 0 &&
                                        <View style={{ marginTop: 5, marginLeft: 10, borderRadius: 10, backgroundColor: PROPERTY.calendarHeaderBackground, height: 43 }} key={'content' + index} >
                                            <View style={{ flexDirection: 'row', alignItems: 'center', margin: 10 }}>
                                                <Text style={{ fontSize: 16, paddingLeft: 9, color: PROPERTY.overdueColor }}>{value.firstname} {value.lastname} : {value.sstatus}</Text>
                                            </View>
                                        </View>
                                    )
                                );
...
export default connect(mapstate)(OverdueBatchEdit);
